...
def circle_exon_count(bamfile2, bedfile, exon_index, split_character, platform):
...
def write_exon_count(outfile, exon_count, sample, circle_id,
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='script to get the coverage profile of all circRNA files in the inputfolder.')
    ...
    output_file = open('%s/%s.exon_counts.bed' % (inputfolder, sample), 'w')
    ...
    for f in files:
        ...
        if f.split('.')[-2] == 'sorted':
            ...
            if len(exon_counts) > 0:
                ...
                format_to_bed12(exon_counts, transcript_id, circle_id, number_of_reads,
                                '%s/%s.exon_counts.bed' % (inputfolder, sample))
