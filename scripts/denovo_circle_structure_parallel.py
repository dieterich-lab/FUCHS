...
def write_single_exon(outfile, coverage, circ_coordinates):
    bed12 = open('%s12.bed' % (outfile), 'a')
    bed6 = open('%s6.bed' % (outfile), 'a')
    if 0 in coverage and sum(coverage) > 0:
        ...
        bed12.write('%s\t%s\t%s\t%s:%s-%s|0|%s\t%s\t.\t%s\t%s\t255,0,0\t2\t%s,%s\t0,%s\n' % (
        circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], circ_coordinates[0], circ_coordinates[1],
        ...
        bed12.write('%s\t%s\t%s\t%s:%s-%s|0|%s\t%s\t.\t%s\t%s\t255,0,0\t1\t%s\t0\n' % (
        circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], circ_coordinates[0], circ_coordinates[1],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='file to run a denonvo exon chain reconstruction. circRNAs are independent from '
                    'each other and script can be run on multiple cores')
    # input
