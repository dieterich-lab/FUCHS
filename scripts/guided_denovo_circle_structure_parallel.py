...
def get_coverage_profile(bamfile, circ_coordinates, transcripts):
    ...
    coordinates = pybedtools.BedTool('%s %s %s' %(circ_coordinates[0], circ_coordinates[1], circ_coordinates[2]), from_string=True)
	...
	if 0 in coverage:
	    transcript_coverage[t]['coverage_breaks'] = [(circ_coordinates[0], circ_coordinates[1] + coverage.index(0), circ_coordinates[1] + len(coverage) - (list(reversed(coverage)).index(0)))]
	else:
	    transcript_coverage[t]['coverage_breaks'] = []
	for i, intron in enumerate(transcripts[t]):
	    if i > 0 and (i+1) < len(transcripts[t]):
		transcript_coverage[t]['introns'][intron] = sum(coverage[intron[1] - circ_coordinates[1]:intron[2]-circ_coordinates[1]])/float(intron[2] - intron[1])
	    if not (i+1) == len(transcripts[t]):
		transcript_coverage[t]['exons'][(intron[0], intron[2], transcripts[t][i+1][1])] = sum(coverage[intron[2] - circ_coordinates[1]:transcripts[t][i+1][1]-circ_coordinates[1]])/float(transcripts[t][i+1][1]-intron[2])
...
def filter_out_exons(transcript_coverage, split_coverage, circ_coordinates): # also keep right side of exon
	...
	for exon in transcript_coverage[t]['exons']:
		...
		if 0 in coverage and not (coverage[0] == 0 or coverage[-1] == 0):
		    ...
		    avg_coverage_right = sum(list(reversed(coverage))[:list(reversed(coverage)).index(0)])/float(list(reversed(coverage)).index(0))
...
def collapse_transcripts(transcript_coverage):
	...
	for i, transcript2 in enumerate(transcript_coverage):
		...
		#print('\n\n')
		if sorted(transcript_coverage[transcript]['exons'].keys()) == sorted(transcript_coverage[transcript2]['exons'].keys()):
...
def infer_missing_structure(transcripts, coordinates, bedfile):
	...
	if len(transcripts[t]['coverage_breaks']) > 0:
		...
		missing_region = pybedtools.BedTool('%s %s %s' %(unsupported[0],unsupported[1], unsupported[2]), from_string=True)
...
def merge_exons(exons):
	...
	for i, e in enumerate(sorted_exons):
		...
		if e[2]+1 >= next_exon[1]: 
		    new_exons[(e[0], e[1], next_exon[2])] = ((exons[e] * (e[2]-e[1]) ) + (exons[next_exon] * (next_exon[2]-next_exon[1])))/ float(next_exon[2] - e[1])
		    merged = [e, next_exon]
		else:
		    new_exons[e] = exons[e]
		...
		if e[1]-1 <= prev_exon[2]:
		    new_exons[(e[0], prev_exon[1], e[2])] = ((exons[e] * (e[2]-e[1]) ) + (exons[prev_exon] * (prev_exon[2]-prev_exon[1])))/ float(e[2] - prev_exon[1])
		    merged = [e, prev_exon]
		else:
		    new_exons[e] = exons[e]
    else:
...
def write_bed12(outfile, transcript_coverage, circ_coordinates, coverage, introns):
    O = open(outfile, 'a')
    for t in transcript_coverage:
	O.write('%s\t%s\t%s\t%s:%s-%s|%s|%s\t' %(circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], t, 1 - (coverage.count(0)/float(len(coverage)))))
	...
	if len(transcript_coverage[t]['introns']) > 0:
	    ...
	    O.write('%s\t.\t%s\t%s\t255,0,0\t%s\t' %(int(sum(transcript_confidence)/float(len(transcript_confidence))), circ_coordinates[1], circ_coordinates[2], len(transcript_coverage[t]['exons'])))
	else:
	    if len(transcript_coverage[t]['exons']) > 0:
		O.write('%s\t.\t%s\t%s\t255,0,0\t%s\t' %(int(sum(transcript_coverage[t]['exons'].values())/float(len(transcript_coverage[t]['exons']))), circ_coordinates[1], circ_coordinates[2], len(transcript_coverage[t]['exons'])))
	    else:
		O.write('0\t.\t%s\t%s\t255,0,0\t%s\t' %(circ_coordinates[1], circ_coordinates[2], len(transcript_coverage[t]['exons'])))
	exon_length = []
	exon_location = []
...
def write_bed6(transcript_coverage, outfile, circ_coordinates, coverage):
    ...
    for e in sorted_exons:
	O.write('%s\t%s\t%s\t%s:%s-%s|%s\t%s\t.\n' %(e[0], e[1], e[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], ','.join(exons[e]), int(sum(coverage[e[1]-circ_coordinates[1]:e[2]-circ_coordinates[1]])/float(e[2]-e[1]))))
    O.close()
    return
...
def write_single_exon(outfile, coverage, circ_coordinates):
	...
	breakpoints = (circ_coordinates[1] + coverage.index(0), circ_coordinates[2] - (list(reversed(coverage)).index(0)))
	exon1 = (circ_coordinates[0], circ_coordinates[1], breakpoints[0])
	exon2 = (circ_coordinates[0], breakpoints[1], circ_coordinates[2])
    	O12.write('%s\t%s\t%s\t%s:%s-%s|0|%s\t%s\t.\t%s\t%s\t255,0,0\t2\t%s,%s\t0,%s\n' %(circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], 1 - (coverage.count(0)/float(len(coverage))), int((sum(coverage[:coverage.index(0)])/float(coverage.index(0)) + sum(coverage[breakpoints[1] - circ_coordinates[1]:])/float(circ_coordinates[2] - breakpoints[1]))/2), circ_coordinates[1], circ_coordinates[2], coverage.index(0), circ_coordinates[2] - breakpoints[1], breakpoints[1] - circ_coordinates[1]))
	O6.write('%s\t%s\t%s\t%s:%s-%s|0\t%s\t.\n' %(exon1[0], exon1[1], exon1[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], int(sum(coverage[:coverage.index(0)])/float(coverage.index(0)))))
	O6.write('%s\t%s\t%s\t%s:%s-%s|0\t%s\t.\n' %(exon2[0], exon2[1], exon2[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], int(sum(coverage[breakpoints[1] - circ_coordinates[1]:])/float(circ_coordinates[2] - breakpoints[1]))))
    elif not 0 in coverage:
	O12.write('%s\t%s\t%s\t%s:%s-%s|0|%s\t%s\t.\t%s\t%s\t255,0,0\t1\t%s\t0\n' %(circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], 1 - (coverage.count(0)/float(len(coverage))), int(sum(coverage)/float(len(coverage))), circ_coordinates[1], circ_coordinates[2], circ_coordinates[2]-circ_coordinates[1]))
	O6.write('%s\t%s\t%s\t%s:%s-%s|0\t%s\t.\n' %(circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], circ_coordinates[0], circ_coordinates[1], circ_coordinates[2], int(sum(coverage)/float(len(coverage)))))
    else:
...
def run_denovo_exon_chain_reconstruction(f, folder, annotation, outfile):
	...
	if len(Introns) > 0:
		...
		TC = {0: {'introns':Introns, 'coverage_breaks': [(circ_coordinates[0], breakpoints[0], breakpoints[1])],'exons':{(circ_coordinates[0], circ_coordinates[1], breakpoints[0]): sum(Cov[:Cov.index(0)])/float(Cov.index(0)), (circ_coordinates[0], breakpoints[1], circ_coordinates[2]): sum(Cov[breakpoints[1] - circ_coordinates[1]:])/float(circ_coordinates[2] - breakpoints[1])}}}
		TC = infer_missing_structure(TC, circ_coordinates, annotation)
		write_bed12('%sinferred_12.bed' %(outfile), TC, circ_coordinates, Cov, Introns)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='file to run a denonvo exon chain reconstruction. circRNAs are independent from each other and script can be run on multiple cores')
    # input
    ...
    # options
    parser.add_argument('-A', dest = 'annotation', default = '.', help = 'if specified, FUCHS will use the annotation to reconstruct the internal structure of unsupported circRNA regions')
    parser.add_argument('-c', dest = 'num_cpus', required=False, type=int, default=multiprocessing.cpu_count(), help='Number of processors to use')
    ...
    O = open('%sinferred_12.bed' %(outfile), 'w')
    O.write('#bed12\n')
    O.close()
    O = open( '%sinferred_6.bed' %(outfile), 'w')
    O.write('#bed6\n')
    O.close()
