...
def iterate_over_folder(inputfolder, bedfile, platform, split_character):
    ...
    for lola in files:
        if lola.split('.')[-1] == 'bam':
            ...
            results[lola.split('.')[0]]['circle_id'] = '%s_%s_%s' % (
...
def write_results(results, outfile):
    ...
    for lola in circles:
        ...
        results[lola]['circle_id'], results[lola]['transcript_ids'], results[lola]['num_reads'],
        results[lola]['min_length'], results[lola]['max_length'], results[lola]['single'], results[lola]['double'],
        results[lola]['undefined']))
    O.close()
    return
